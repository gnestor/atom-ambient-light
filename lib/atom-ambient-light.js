...
function getActiveTheme() {
  return atom.themes
    .getActiveThemes()
    .find(theme => !theme.stylesheets[0][1].includes('atom-workspace'));
}
...
function invertColor(value, percentage = 1) {
  try {
    ...
    if (value.includes('!important')) {
      const importantColor = Color(value.replace(' !important', ''))
        .mix(Color(value.replace(' !important', '')).negate(), percentage)
        .string();
...
function transformAst(ast, lightLevel) {
  ...
  const background = ast.stylesheet.rules
    .find(rule => rule.selectors && rule.selectors.includes('atom-text-editor'))
    .declarations.find(declaration =>
      declaration.property.includes('background')).value;
  ...
  return {
    ...
    stylesheet: {
      ...
      rules: ast.stylesheet.rules &&
        ast.stylesheet.rules.map(rule => ({
          ...rule,
          declarations: rule.declarations &&
            rule.declarations.reduce(
              (result, declaration) => {
                if (
                  ...
                  return [
                    ...
                    {
                      ...declaration,
                      value: invertColor(declaration.value, percentage)
                    }
                  ];
                }
