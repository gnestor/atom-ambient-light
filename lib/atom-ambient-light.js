...
function invertColors(value, percentage = 1) {
  try {
    if (value === 'inherit') return value;
    if (value.includes('!important')) {
      return `${Color(value.replace(' !important', '')).mix(Color(value.replace(' !important', '')).negate(), percentage).hex()} !important`;
    }
    return Color(value).mix(Color(value).negate(), percentage).hex();
  } catch (error) {
    return value;
  }
}
...
function transformAst(ast, lightLevel) {
  ...
  .find(rule => rule.selectors.includes('atom-text-editor'))
  .declarations
  .find(declaration => declaration.property.includes('background'))
  .value;
  ...
  const percentage = Color(background).dark() ? (1 - normalize(lightLevel)) : normalize(lightLevel);
  return {
    ...ast,
    stylesheet: {
      ...ast.stylesheet,
